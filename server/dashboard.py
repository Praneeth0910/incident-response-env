...
def run_task(difficulty: str) -> str:
    """Simulates running an RL task at the given difficulty."""
    import random, time

    steps = 10
    reward_map = {"EASY": (0.7, 1.0), "MEDIUM": (0.3, 0.8), "HARD": (0.0, 0.6)}
    lo, hi = reward_map.get(difficulty, (0.0, 1.0))

    lines = [
        f"[SYSTEM] Starting {difficulty} task...",
        f"[AGENT]  Initializing policy...",
    ]
    total_reward = 0.0
    for step in range(1, steps + 1):
        r = round(random.uniform(lo, hi), 4)
        total_reward += r
        lines.append(f"[STEP {step:02d}] reward={r:.4f}  cumulative={total_reward:.4f}")

    solved = total_reward / steps >= 0.5
    status = "✅ SOLVED" if solved else "❌ FAILED"
    lines.append(f"[RESULT] {status}  avg_reward={total_reward/steps:.4f}")
    return "\n".join(lines)


def run_benchmark(model: str, endpoint: str) -> str:
    """Simulates a benchmark run against the selected model/endpoint."""
    import random
    ...
    lines = [
        f"[BENCH] Model:    {model}",
        f"[BENCH] Endpoint: {endpoint}",
        f"[BENCH] Running 3 tasks...",
        "",
    ]
    scores = []
    ...
    lines += [
        "",
        f"[BENCH] AVG SCORE: {avg:.4f}",
        f"[BENCH] Tasks solved: {sum(1 for s in scores if s >= 0.5)}/3",
        "[BENCH] Done.",
    ]
    return "\n".join(lines)
...
╚═╝╚═╝  ╚═══╝ ╚═════╝╚═╝╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("/dashboard"):
                ...
                # Wire buttons → log output
                btn_easy.click(fn=lambda: run_task("EASY"),   inputs=[], outputs=[log_out])
                btn_med.click( fn=lambda: run_task("MEDIUM"), inputs=[], outputs=[log_out])
                btn_hard.click(fn=lambda: run_task("HARD"),   inputs=[], outputs=[log_out])
